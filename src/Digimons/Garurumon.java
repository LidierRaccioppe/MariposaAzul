...
public class Garurumon extends Digimon{
    ...
    public Garurumon(String nombre, String etapaDigievolutiva, float maxHP, float vidaActual, int nivel, int XP, float fuerza, float defensa, float agilidad) {
        super(nombre, etapaDigievolutiva, maxHP, vidaActual, nivel, XP, fuerza, defensa, agilidad);
    }

    public int ataques(int seleccion){
        int daño= 0;
        //verifica si se eliigio con exito un ataque
    ...
    public Digimon crecimientoDigimon(Digimon digimonPorCrecer, int restaEntreNivelOriginalYNivelObtenido){
        ...
        if(restaEntreNivelOriginalYNivelObtenido!=0){
            ...
            digimonCrecido.setAgilidad(digimonPorCrecer.getAgilidad()*+(7 * restaEntreNivelOriginalYNivelObtenido));
        };
